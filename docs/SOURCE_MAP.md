...
| [tensor_dynamic_ops.pmi](../runtime/tensor_dynamic_ops.pmi) | Runtime-dimension wrappers for those kernels: shape and type checks at run time, outputs, parameters; and the runtime-dimension Relu, Flatten and BatchNormalization, over the fixed-shape kernels. One source for both dialects. |
