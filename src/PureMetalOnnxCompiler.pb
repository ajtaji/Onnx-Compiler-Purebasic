...
XIncludeFile "compiler/onnx_forms.pbi"
...
Procedure PrintUsage()
  ...
  PrintN("  Windows: the program uses every processor its process may use (found when the model is bound);")
  PrintN("  --threads N lowers that to at most N (1 = the calling thread alone). Same output bits at any count.")
  PrintN("  --random-inputs makes every RandomNormal/RandomUniform(-Like), Bernoulli and Multinomial node an extra model input the caller fills.")
