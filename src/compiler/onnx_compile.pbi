...
Declare.i PmdExportTargetRuntime(Folder.s,TargetIndex.i)
    ...
    If Known = 0
      ...
      ForEach *Model\Graph\Nodes()\Inputs()
        ...
        Select Op
          ...
          Case "Reshape", "Expand", "Squeeze", "Unsqueeze", "ReduceMean", "ReduceSum", "CumSum" : Control = Bool(Position = 1)
...
Procedure.i PmoCompileValidate(*Ir.PmoIrModel)
  ...
  ForEach *Ir\Source\Opsets()
    ...
    EndIf
  Next
  If DefaultOpset < 7 Or DefaultOpset > 23
...
Procedure.i PmoCompileCommand(ModelPath.s)
  ...
  If PmoTargets(TargetIndex)\Launch = #PMO_LAUNCH_EXTERNAL_MEMORY
    ...
    EndIf
  EndIf
  If PmdExportTargetRuntime(OutputPrefix+".runtime/",TargetIndex)=0 : PmoCompileFail(PmoDynamicError) : Goto PmoCompileCommandFailed : EndIf
