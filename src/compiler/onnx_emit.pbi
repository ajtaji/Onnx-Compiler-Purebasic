...
Procedure.i PmoEmitBatchedMatMulHelper(File.i, *Ir.PmoIrModel, *Profile.PmoTargetProfile,
  ...
  For Rank = 0 To PmoEmitRank(*Out) - 3 : AddElement(TempOut\Dims()) : TempOut\Dims() = PmoEmitDim(*Out, Rank) : Next
  IA = PmoEmitBroadcastIndex("batch", @TempA, @TempOut)
  IB = PmoEmitBroadcastIndex("batch", @TempB, @TempOut)
  M = PmoEmitDim(*A, PmoEmitRank(*A) - 2) : K = PmoEmitDim(*A, PmoEmitRank(*A) - 1)
  N = PmoEmitDim(*B, PmoEmitRank(*B) - 1) : Batches = PmoEmitProduct(*Out, 0, PmoEmitRank(*Out) - 3)
  PmoEmitLine(File, "Procedure " + ProcName + "(*a, *b, *dst)")
...
Procedure.i PmoEmitCastHelper(File.i, *Ir.PmoIrModel, *Ref.PmoIrNodeRef, Map Calls.s())
  Protected *Node.PmoOnnxNode = *Ref\Node
  Protected *Src.PmoIrValue = PmoEmitValue(*Ir, PmoEmitInput(*Node, 0))
  Protected *Out.PmoIrValue = PmoEmitValue(*Ir, PmoEmitOutput(*Node, 0))
  Protected ProcName.s = "PmOnnxNode" + Str(*Ref\Index)
  ...
  If *Src\ElementType = 1
    ...
    If *Out\ElementType = 7
      PmoEmitLine(File, "    iv = fv") : PmoEmitLine(File, "    " + PmoEmitPut(7, "*dst", "i", "iv"))
...
Procedure.i PmoEmitPadHelper(File.i, *Ir.PmoIrModel, *Ref.PmoIrNodeRef, Map Calls.s())
  Protected *Node.PmoOnnxNode = *Ref\Node
  Protected *Src.PmoIrValue = PmoEmitValue(*Ir, PmoEmitInput(*Node, 0))
  Protected *Out.PmoIrValue = PmoEmitValue(*Ir, PmoEmitOutput(*Node, 0))
  Protected ProcName.s = "PmOnnxNode" + Str(*Ref\Index)
  Protected PadsName.s = PmoEmitInput(*Node, 1)
  Protected AxesName.s = PmoEmitInput(*Node, 3)
  Protected *Pads.PmoIrConstant = PmoEmitConstant(*Ir, PadsName)
  Protected *Axes.PmoIrConstant = PmoEmitConstant(*Ir, AxesName)
  ...
  Protected Mode.s = PmoEmitAttrS(*Node, "mode", "constant")
  Protected Fill.f
  If *Src = 0 Or *Out = 0 Or *Pads = 0 : ProcedureReturn PmoEmitFail("Pad has missing static values") : EndIf
  AxisCount = PmoEmitRank(*Src) : If *Axes : AxisCount = *Axes\Elements : EndIf
  For Pos = 0 To AxisCount - 1
    Axis = Pos : If *Axes : Axis = PmoEmitConstI(*Ir, AxesName, Pos, @Ok) : EndIf
    If Axis < 0 : Axis + PmoEmitRank(*Src) : EndIf
    If Axis < 0 Or Axis >= PmoEmitRank(*Src) : ProcedureReturn PmoEmitFail("Pad axis is outside rank") : EndIf
    If Axis = PmoEmitRank(*Src) - 1
      Before = PmoEmitConstI(*Ir, PadsName, Pos, @Ok)
      After = PmoEmitConstI(*Ir, PadsName, Pos + AxisCount, @Ok)
    ElseIf PmoEmitConstI(*Ir, PadsName, Pos, @Ok) <> 0 Or
           PmoEmitConstI(*Ir, PadsName, Pos + AxisCount, @Ok) <> 0
      ProcedureReturn PmoEmitFail("Pad changes a non-final axis")
    EndIf
  Next
  If PmoEmitInput(*Node, 2) <> "" : Fill = PmoEmitConstF(*Ir, PmoEmitInput(*Node, 2), 0, @Ok) : EndIf
  Rows = PmoEmitProduct(*Src, 0, PmoEmitRank(*Src) - 2)
  InWidth = PmoEmitDim(*Src, PmoEmitRank(*Src) - 1) : OutWidth = PmoEmitDim(*Out, PmoEmitRank(*Out) - 1)
  PmoEmitLine(File, "Procedure " + ProcName + "(*src, *dst)")
  PmoEmitLine(File, "  Protected row.i") : PmoEmitLine(File, "  Protected ox.i")
  PmoEmitLine(File, "  Protected ix.i") : PmoEmitLine(File, "  row = 0")
  PmoEmitLine(File, "  While row < " + Str(Rows)) : PmoEmitLine(File, "    ox = 0")
  PmoEmitLine(File, "    While ox < " + Str(OutWidth))
  PmoEmitLine(File, "      ix = ox - " + Str(Before))
  If Mode = "reflect"
    PmoEmitLine(File, "      If ix < 0 : ix = 0 - ix : EndIf")
    PmoEmitLine(File, "      If ix >= " + Str(InWidth) + " : ix = " + Str(2 * InWidth - 2) + " - ix : EndIf")
    PmoEmitLine(File, "      PmTensorPut(*dst, row * " + Str(OutWidth) + " + ox, PmTensorGet(*src, row * " + Str(InWidth) + " + ix))")
  ElseIf Mode = "edge"
    PmoEmitLine(File, "      If ix < 0 : ix = 0 : EndIf")
    PmoEmitLine(File, "      If ix >= " + Str(InWidth) + " : ix = " + Str(InWidth - 1) + " : EndIf")
    PmoEmitLine(File, "      PmTensorPut(*dst, row * " + Str(OutWidth) + " + ox, PmTensorGet(*src, row * " + Str(InWidth) + " + ix))")
  Else
    PmoEmitLine(File, "      If ix < 0 Or ix >= " + Str(InWidth))
    PmoEmitLine(File, "        PmTensorPut(*dst, row * " + Str(OutWidth) + " + ox, " + PmoEmitFloat(Fill) + ")")
    PmoEmitLine(File, "      Else")
    PmoEmitLine(File, "        PmTensorPut(*dst, row * " + Str(OutWidth) + " + ox, PmTensorGet(*src, row * " + Str(InWidth) + " + ix))")
    PmoEmitLine(File, "      EndIf")
  EndIf
  PmoEmitLine(File, "      ox = ox + 1") : PmoEmitLine(File, "    Wend")
  PmoEmitLine(File, "    row = row + 1") : PmoEmitLine(File, "  Wend")
  PmoEmitLine(File, "EndProcedure") : PmoEmitLine(File)
  Calls(Str(*Ref\Index)) = ProcName
  ProcedureReturn Bool(PmoEmitError = "")
EndProcedure
  ...
  ForEach *Ir\Nodes()
    ...
    Select Op
      Case "Add", "Sub", "Mul", "Div", "Pow"
        If PmoEmitBinaryHelper(File, *Ir, @*Ir\Nodes(), Calls()) = 0 : ProcedureReturn #False : EndIf
      Case "Reshape"
      ...
      Case "Cast"
        If PmoEmitCastHelper(File, *Ir, @*Ir\Nodes(), Calls()) = 0 : ProcedureReturn #False : EndIf
      Case "Range"
      ...
      Case "Pad"
        If PmoEmitPadHelper(File, *Ir, @*Ir\Nodes(), Calls()) = 0 : ProcedureReturn #False : EndIf
      Case "NonZero"
...
Procedure.i PmoEmitNode(File.i, *Ir.PmoIrModel, *Profile.PmoTargetProfile,
  ...
  *Y = PmoEmitValue(*Ir, PmoEmitOutput(*Node, 0))
  ...
  If FindMapElement(Calls(), Str(*Ref\Index))
    ...
    Next
    If Params <> "" : Params + ", " : EndIf : Params + Out(0)
  ...
  ElseIf Op = "MatMul"
    *A = PmoEmitValue(*Ir, PmoEmitInput(*Node, 0)) : *B = PmoEmitValue(*Ir, PmoEmitInput(*Node, 1))
    M = PmoEmitDim(*A, 0) : K = PmoEmitDim(*A, 1) : N = PmoEmitDim(*B, 1)
    *Quant = PmoEmitQuantWeight(*Ir, PmoEmitInput(*Node, 1))
    If *Quant
  ...
  ElseIf Op = "Gemm"
    *A = PmoEmitValue(*Ir, PmoEmitInput(*Node, 0)) : *B = PmoEmitValue(*Ir, PmoEmitInput(*Node, 1))
    ...
    PmoEmitLine(File, "  PmOnnxGemm\C = " + In(2)) : PmoEmitLine(File, "  PmOnnxGemm\Dst = " + Out(0))
  ...
  ElseIf Op = "Softmax"
    Width = PmoEmitDim(PmoEmitValue(*Ir, PmoEmitInput(*Node, 0)), PmoEmitRank(PmoEmitValue(*Ir, PmoEmitInput(*Node, 0))) - 1)
    PmoEmitLine(File, "  PmTensorSoftmaxLast(" + In(0) + ", " + Out(0) + ", " + Str(Count / Width) + ", " + Str(Width) + ")")
  ...
  ElseIf Op = "LayerNormalization"
    *A = PmoEmitValue(*Ir, PmoEmitInput(*Node, 0)) : Rank = PmoEmitRank(*A) : Width = PmoEmitDim(*A, Rank - 1)
    Bias = "0" : If InputCount > 2 : Bias = In(2) : EndIf
    PmoEmitLine(File, "  PmTensorLayerNormLast(" + In(0) + ", " + In(1) + ", " + Bias + ", " + Out(0) + ", " +
                      Str(PmoEmitProduct(*A, 0, Rank - 2)) + ", " + Str(Width) + ", " +
                      PmoEmitFloat(PmoEmitAttrF(*Node, "epsilon", 0.00001)) + ")")
  ElseIf Op = "Clip"
    If InputCount > 1 And PmoEmitInput(*Node, 1) <> "" : Lo = PmoEmitConstF(*Ir, PmoEmitInput(*Node, 1), 0, @Ok) : Else : Lo = PmoEmitAttrF(*Node, "min", Lo) : EndIf
    If InputCount > 2 And PmoEmitInput(*Node, 2) <> "" : Hi = PmoEmitConstF(*Ir, PmoEmitInput(*Node, 2), 0, @Ok) : Else : Hi = PmoEmitAttrF(*Node, "max", Hi) : EndIf
    PmoEmitLine(File, "  PmTensorClip(" + In(0) + ", " + Out(0) + ", " + Str(Count) + ", " + PmoEmitFloat(Lo) + ", " + PmoEmitFloat(Hi) + ")")
  ...
  ElseIf Op = "ConvTranspose"
    *A = PmoEmitValue(*Ir, PmoEmitInput(*Node, 0)) : *B = PmoEmitValue(*Ir, PmoEmitInput(*Node, 1))
    ...
    PmoEmitLine(File, "  PmOnnxConvTranspose1\PadLeft = " + Str(PmoEmitAttrListI(*Node, "pads", 0, 0)))
    PmoEmitLine(File, "  PmOnnxConvTranspose1\Stride = " + Str(PmoEmitAttrListI(*Node, "strides", 0, 1)))
    PmoEmitLine(File, "  PmOnnxConvTranspose1\Dilation = " + Str(PmoEmitAttrListI(*Node, "dilations", 0, 1)))
