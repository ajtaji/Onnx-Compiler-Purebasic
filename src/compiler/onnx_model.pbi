...
; What a ValueInfoProto's TypeProto holds. A tensor keeps its element type and
; shape in ElementType/Dims as before. A sequence of tensors records only its
; element type: HasTensorType and HasShape stay zero for it, so no path that
; reads tensor shapes can mistake a sequence for a tensor.
#PMO_VALUE_TENSOR = 0
#PMO_VALUE_SEQUENCE = 1
#PMO_VALUE_OPTIONAL = 2
#PMO_VALUE_OTHER = 3
...
Structure PmoOnnxAttribute
  ...
  ; A GRAPH attribute (If's branches, Loop's body): the address of a
  ; PmoOnnxGraph this attribute owns, zero otherwise. An address rather than a
  ; typed pointer only because the graph structure is declared below this one.
  ; PmoOnnxFreeGraphChildren releases it.
  Graph.i
  List Floats.f()
  List Integers.q()
  List Strings.s()
...
; TypeProto.Sequence: field 1 is the element TypeProto. Only a sequence of
; tensors is recognised; a sequence of sequences, maps or optionals is marked
; #PMO_VALUE_OTHER so the emitters refuse it by name.
Procedure.i PmoOnnxParseSequenceType(*Cursor.PmoWireCursor, *Value.PmoOnnxValue)
  Protected Field.Integer
  Protected Wire.Integer
  Protected Sub.PmoWireCursor
  Protected Inner.Integer
  Protected InnerWire.Integer
  Protected Element.PmoOnnxValue
  Protected TensorSub.PmoWireCursor
  *Value\ValueKind = #PMO_VALUE_SEQUENCE
  While PmoWireRemaining(*Cursor) > 0
    If PmoWireReadKey(*Cursor, @Field, @Wire) = 0 : ProcedureReturn #False : EndIf
    If Field\i = 1
      If PmoWireRequire(*Cursor, Field\i, 2, Wire\i) = 0 Or PmoWireReadSubmessage(*Cursor, @Sub) = 0
      ...
      EndIf
      While PmoWireRemaining(@Sub) > 0
        If PmoWireReadKey(@Sub, @Inner, @InnerWire) = 0 : ProcedureReturn #False : EndIf
        Select Inner\i
          Case 1
            If PmoWireRequire(@Sub, Inner\i, 2, InnerWire\i) = 0 Or PmoWireReadSubmessage(@Sub, @TensorSub) = 0
              ProcedureReturn #False
            EndIf
            If PmoOnnxParseTensorType(@TensorSub, @Element) = 0 : ProcedureReturn #False : EndIf
            *Value\SequenceElementType = Element\ElementType
          Case 4, 5, 8, 9
            *Value\ValueKind = #PMO_VALUE_OTHER
            If PmoWireSkip(@Sub, InnerWire\i) = 0 : ProcedureReturn #False : EndIf
          Default
            If PmoWireSkip(@Sub, InnerWire\i) = 0 : ProcedureReturn #False : EndIf
        EndSelect
      Wend
...
Procedure.i PmoOnnxParseType(*Cursor.PmoWireCursor, *Value.PmoOnnxValue)
  ...
  While PmoWireRemaining(*Cursor) > 0
    ...
    Select Field\i
      ...
      Case 4 ; sequence_type
        If PmoWireRequire(*Cursor, Field\i, 2, Wire\i) = 0 Or PmoWireReadSubmessage(*Cursor, @Sub) = 0
          ProcedureReturn #False
        EndIf
        If PmoOnnxParseSequenceType(@Sub, *Value) = 0 : ProcedureReturn #False : EndIf
      Case 9 ; optional_type
        *Value\ValueKind = #PMO_VALUE_OPTIONAL
        If PmoWireSkip(*Cursor, Wire\i) = 0 : ProcedureReturn #False : EndIf
      Case 5, 8 ; map_type, sparse_tensor_type
        *Value\ValueKind = #PMO_VALUE_OTHER
        If PmoWireSkip(*Cursor, Wire\i) = 0 : ProcedureReturn #False : EndIf
      Default
        If PmoWireSkip(*Cursor, Wire\i) = 0 : ProcedureReturn #False : EndIf
...
Procedure.i PmoOnnxParseAttribute(*Cursor.PmoWireCursor, *Attribute.PmoOnnxAttribute)
  ...
  While PmoWireRemaining(*Cursor) > 0
    ...
    Select Field\i
      ...
      Case 6 ; g - one subgraph (an If branch, a Loop body)
        If PmoWireRequire(*Cursor, Field\i, 2, Wire\i) = 0 Or PmoWireReadSubmessage(*Cursor, @Sub) = 0
          ProcedureReturn #False
        EndIf
        If *Attribute\Graph
          ProcedureReturn PmoWireFail("AttributeProto carries more than one subgraph")
        EndIf
...
; Releases every subgraph the attributes of a graph own, depth first. The
; top-level graph lives inside PmoOnnxModel and is cleared with it; nested
; graphs were allocated by PmoOnnxParseGraphAddress and are freed here.
Procedure PmoOnnxFreeGraphChildren(*Graph.PmoOnnxGraph)
  Protected *Child.PmoOnnxGraph
  If *Graph = 0 : ProcedureReturn : EndIf
  ForEach *Graph\Nodes()
    ForEach *Graph\Nodes()\Attributes()
      If *Graph\Nodes()\Attributes()\Graph
        *Child = *Graph\Nodes()\Attributes()\Graph
        PmoOnnxFreeGraphChildren(*Child)
        FreeStructure(*Child)
        *Graph\Nodes()\Attributes()\Graph = 0
      EndIf
    Next
  Next
EndProcedure

; Parses one nested GraphProto into a newly allocated structure and returns its
; address, or zero with PmoWireError set. A partly parsed graph is released
; here, so a failure never leaves an orphan behind.
Procedure.i PmoOnnxParseGraphAddress(*Cursor)
  Protected *Graph.PmoOnnxGraph = AllocateStructure(PmoOnnxGraph)
  If *Graph = 0
    PmoWireFail("cannot allocate a subgraph")
    ProcedureReturn 0
  EndIf
  If PmoOnnxParseGraph(*Cursor, *Graph) = 0
    PmoOnnxFreeGraphChildren(*Graph)
    FreeStructure(*Graph)
    ProcedureReturn 0
  EndIf
...
Procedure PmoOnnxFree(*Model.PmoOnnxModel)
  ...
  PmoOnnxFreeGraphChildren(@*Model\Graph)
  If *Model\FileData
